addspeciation <- function (n,xmin,time,node.new,node,r,N,edge,edge.length,events,rootdistr,rootdistro,rootniche,rootback,rootu,roota,rootb,rootx,nspecies,PO,PA,sd,d,bias,area,lik.old) {
type <- 1
a <- a2 <- r
b <- b2 <- r
anc <- edge[node,1]
...
if (time==0) {
if (xmin>0) {
node2 <- setdiff(which(edge[,1]==edge[node.new,1]),node.new)
tmp <- events
tmp[nrow(tmp)+(1:2),] <-rbind(c(node.new,type,time,a,b,n,xmin),c(node2,type,time,a2,b2,-n,xmin))
} else {
tmp <- events
...
tmp <- events
tmp[nrow(tmp)+1,] <- c(node.new,type,time,a,b,n,abs(xmin))
tmp <- tmp[with(tmp,order(node,time)),]
