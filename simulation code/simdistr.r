simdistr <- function (events,edge,edge.length,nspecies,N,rootu,sd,roota,rootb,rootx,rootdistr,rootdistro,d,area,PAloc,nBK) {
upd <- function (events,edget,dendistrt,dendistrto,a,b,u,x,sd,N,d) {
  ...
  if (events$time[1]>0) {
  	...
  	if (is.na(sum(tmp))) {
  		stop(paste(paste(events[z,],collapse=","),"too large a or b"))
  	}
  ...
  result <- list(events=events,dendistrt=dendistrt,dendistrto=dendistrto,a=a,b=b,u=u,x=x)
  result
}
##evolve niches along phylogeny
    ...
    j <- des[1]
    res <- upd(events[events$node==j,],edget=edge.length[j],dendistrt=rootdistr,dendistrto=rootdistro,a=roota,b=rootb,u=rootu,x=rootx,sd,N,d)
...
##extract tip species distribution data
	niche <- matrix(NA,nspecies,N)
	for (i in c(1:nspecies)) {
		a <- avec[as.character(i)]
		b <- bvec[as.character(i)]
		tmp <- sapply((seq(1:N)/(exp(a)+0.1))^(exp(b)+0.1),gammainc,a=1/(exp(b)+0.1))
		nichedistrt <- (tmp-c(0,tmp[-length(tmp)]))/2/gamma(1/(exp(b)+0.1))
		nichedistrt <- nichedistrt/nichedistrt[1]
		niche[i,] <- nichedistrt
	}
    back <- cbind(pnorm(1,mean=uvec[as.character(1:nspecies)],sd=sd),sapply(seq(2,N-1,1),pnorm,mean=uvec[as.character(1:nspecies)],sd=sd)-sapply(seq(1,N-2,1),pnorm,mean=uvec[as.character(1:nspecies)],sd=sd),1-pnorm(N-1,mean=uvec[as.character(1:nspecies)],sd=sd))
    BK <- lapply(1:nspecies,function (i) sample.int(n=N,size=nBK,replace=T,prob=back[i,]))
    for (i in 1:nspecies) {back[i,0:(xvec[as.character(i)]-1)] <- 0}
    rateall <- area*rowSums(back*niche)
    ...
    out <- list(PO=PO,PA=PA,BK=BK,events=events,dendistr=dendistr,backdistr=back,nichedistr=niche,avec=avec,bvec=bvec,uvec=uvec,xvec=xvec)
    out
}