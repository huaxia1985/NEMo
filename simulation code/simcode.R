simcode <- function (sp,mu,nspecies,A1,A2,rootu,sd,roota,rootb,rootx,d,N,expinv.a,expinv.b,pois.x,area,nBK) {
	...
	events$n[(k1+1):(k1+k2)] <- sample.int((N-1),size=k2,replace=T)
	events[(k1+k2+1):(k1+k2+length(tmp)),] <- cbind(sis,1,0,NA,NA,-events$n[1:length(tmp)],NA)
	events$a <- sample(expinv.a,nrow(events),replace=T)
	events$b <- sample(expinv.b,nrow(events),replace=T)
	events$xmin <- rpois(nrow(events),lambda=pois.x)+1
	events <- events[with(events,order(node,time)),]
	PAloc <- sample.int(10,1000,replace=T)
	sim <- try(simdistr(events=events,edge=tree$edge,edge.length=tree$edge.length,nspecies=nspecies,N=N,rootu=rootu,sd=sd,roota=roota,rootb=rootb,rootx=rootx,rootdistr=rootdistr,rootdistro=rootdistro,d=d,area=area,PAloc=PAloc,nBK=nBK),silent=T)
